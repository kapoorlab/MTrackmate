...
import static fiji.plugin.mtrackmate.gui.Icons.MAGNIFIER_ICON;
...
import fiji.plugin.mtrackmate.visualization.trackscheme.SpotIconGrabber;
...
public class ExtractTrackStackAction extends AbstractTMAction
{
			...
			final Frame parent )
	{
		...
		if ( nspots != 2 )
		{
			if ( nspots == 1 )
			{
				...
				// Get stack.
				final ImagePlus imp = trackStack( mtrackmate, spot, do3d, logger );
				imp.show();
		...
		else
		{
			...
			// Get stack.
			final ImagePlus imp = trackStack( mtrackmate, start1, end1, do3d, logger );
			imp.show();
			imp.setZ( imp.getNSlices() / 2 + 1 );
			imp.resetDisplayRange();
		}
			...
			final Logger logger )
	{
		...
		final Spot start = spots.get( 0 );
		final Spot end = spots.get( spots.size() - 1 );
		return trackStack( mtrackmate, start, end, do3d, logger );
	}
			...
			final Logger logger )
	{
		...
		return trackStack( mtrackmate.getSettings(), path, radius, do3d, logger );
	}
