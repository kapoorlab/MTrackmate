...
import fiji.plugin.mtrackmate.detection.util.MedianFilter2D;
		...
		new Thread( "TrackMate preview detection thread" )
		{
			...
			public void run()
			{
				try
				{
					...
					final TrackMate mtrackmate = new TrackMate( lSettings );
					mtrackmate.getModel().setLogger( logger );

					final boolean detectionOk = mtrackmate.execDetection();
					if ( !detectionOk )
					{
						logger.error( mtrackmate.getErrorMessage() );
						return;
					}
					logger.log( "Found " + mtrackmate.getModel().getSpots().getNSpots( false ) + " spots." );

					// Wrap new spots in a list.
					final SpotCollection newspots = mtrackmate.getModel().getSpots();
